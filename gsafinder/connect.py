"""Native cognis-connect emit for gsafinder — forward findings to any platform.

Maps gsafinder's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    gsafinder ... --format json | gsafinder-emit --to stix
    gsafinder-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
