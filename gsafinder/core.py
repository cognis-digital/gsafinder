...
TOOL_NAME = "GSAFINDER"
TOOL_VERSION = "0.1.0"
RULES = [{'id': 'GSA-MATCH-001', 'severity': 'info', 'weight': 1.0, 'title': 'NAICS_PSC_MATCH', 'when': {'naics_match': True, 'psc_match': True}, 'remediation': 'Review opportunity; assess bid effort.'}, {'id': 'GSA-SETAS-001', 'severity': 'info', 'weight': 1.0, 'title': 'SET_ASIDE_OPPORTUNITY', 'when': {'set_aside': ['SDVOSB', 'WOSB', 'HUBZone', '8(a)']}, 'remediation': 'Confirm eligibility; capture-team alert.'}]
        ...
        for rule in RULES:
            if _match(item, rule):
                result.add(Finding(
                    ...
                    location=str(target),
                    remediation=rule.get("remediation",""), category="gsa",
                    metadata=item,
                ))
